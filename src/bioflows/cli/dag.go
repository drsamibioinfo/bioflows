...
func RenderGraphViz(pipelineFile string,bfLogger *logo.Logger) (string,error) {
	...
	if !helpers.IsValidUrl(pipelineFile) {
		...
		if err != nil {
			bfLogger.Error(fmt.Sprintf("Error Downloading the file: %s",err.Error()))
			return "", err
		}
	}
